...
export function createTranscriptionConfirmationMessage({
  ...
  const parts = [createCloudTranscriptionConfirmationMessage(fileName, extractsAudio)];
  ...
  if (shouldConfirmLongMedia(durationSeconds, longThresholdSeconds)) {
    parts.push(`目前媒體長度約 ${formatConfirmationDuration(durationSeconds)}，線上轉錄會依音訊長度消耗用量。正式測試建議先用 10 秒音訊樣本。`);
  }
